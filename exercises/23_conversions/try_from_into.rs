...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if arr.iter().all(|&x| x >= 0 && x <= 255) {
            let red = arr[0].try_into().unwrap();
            let green = arr[1].try_into().unwrap();
            let blue = arr[2].try_into().unwrap();
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            ...
            if slice.iter().all(|&x| x >= 0 && x <= 255) {
                let red = slice[0].try_into().unwrap();
                let green = slice[1].try_into().unwrap();
                let blue = slice[2].try_into().unwrap();
                return Ok(Color { red, green, blue });
            } else {
                return Err(IntoColorError::IntConversion);
            }
